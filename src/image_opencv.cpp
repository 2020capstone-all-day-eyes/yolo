...
char* translation(char* bname) {
    if (!strcmp(bname, "people") || !strcmp(bname, "person")) {
        return "사람이 있습니다";
    }
    else if (!strcmp(bname, "car")) {
        return "자동차가 있습니다";
    }
    else if (!strcmp(bname, "bus")) {
        return "버스가 있습니다";
    }
    else if (!strcmp(bname, "bicycle")) {
        return "자전거가 있습니다";
    }
    else if (!strcmp(bname, "motorcycle")) {
        return "오토바이가 있습니다";
    }
    else if (!strcmp(bname, "Bench")) {
        return "벤치가 있습니다";
    }
    else if (!strcmp(bname, "bollard")) {
        return "볼라드가 있습니다";
    }
    else if (!strcmp(bname, "backpack")) {
        return "가방이 있습니다";
    }
    else if (!strcmp(bname, "cell phone")) {
        return "휴대폰이 있습니다";
    }
    else if (!strcmp(bname, "umbrella")) {
        return "우산이 있습니다";
    }
    else if (!strcmp(bname, "utilitypole")) {
        return "전봇대가 있습니다";
    }
    else if (!strcmp(bname, "stairs")) {
        return "계단이 있습니다";
    }
    else if (!strcmp(bname, "safetycone")) {
        return "안전 고깔이 있습니다";
    }
    else if (!strcmp(bname, "standingboard")) {
        return "입간판이 있습니다";
    }
    else if (!strcmp(bname, "fire hydrant")) {
        return "소화전이 있습니다";
    }
    else if (!strcmp(bname, "crosswalk")) {
        return "횡단 보도가 있습니다";
    }
    else {
        return bname;
...
void sound(char* bm) {
    ...
    if (!strcmp(bm, "X")) {
//        dtext = fopen("detection_text.txt", "w");
//        fputs("전방에 감지되는 물체가 없습니다", dtext);
//        젯슨나노 sound0.py 경로
//        system("python /home/nano/darknet/sound/sound0.py");
    }
    else {
        FILE* dtext;

        dtext = fopen("detection_text.txt", "a");
        fputs(bm, dtext);

        fclose(dtext);

        //system("python /home/nano/TTS/synthesize_file.py --text /home/nano/darknet/detection_text.txt");
        //젯슨나노 TTS 작동 코드
    }
    return;
}

void inputitt(cv::Mat show_img, int left, int right, int top, int bot) {
     cv::Mat image(show_img);

//     젯슨나노 ITT, TTS 작동 부분
//     cv::imwrite("/home/nano/ITT/detecim.jpg", image);
//     system("python /home/nano/ITT/vision_test.py text /home/nano/ITT/detecim.jpg");
//     system("python /home/nano/TTS/synthesize_file.py --text /home/nano//ITT/text.txt");
}

int centercheck(cv::Mat show_img, int left, int right, int top, int bot) {
     cv::Mat image(show_img);

     if (right < (image.cols / 4) || left >((image.cols / 4) * 3)) {
          return 0;
     }
     return 1;
}

int Distance(int botmax) {
//    m1과 m2의 픽셀값은 사용하는 기기에 맞게 변경
    int m1 = 1500, m2 = 1900;
    if (botmax >= m2) {
        return 2;
//        2미터 이내
...
// Draw Detection
// ====================================================================
void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, int ccc)
{
    try {
        cv::Mat *show_img = mat;
        char* namecatch = NULL;
        int* bottomlist = (int*)malloc(sizeof(int) * num);
        char** bottomlist_name = (char**)malloc(sizeof(char) * num * 10);
        int ittcount = 0, centerobnum = 0, bc, botmax = 0, botmax_number = 0, i, j;
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                ...
                if (ittcount == 1 && ccc == 15) {
                    inputitt(*show_img, left, right, top, bot);
                    ittcount = 0;
                }

                if (centercheck(*show_img, left, right, top, bot)) {
                    ...
                    bottomlist[centerobnum] = bot;
                    bottomlist_name[centerobnum] = namecatch;
        ...
        if (ccc >= 25) {
            if (centerobnum != 0) {
                for (bc = 0; bc < centerobnum; bc++) {
                    if (botmax < bottomlist[bc]) {
                        botmax = bottomlist[bc];
                        botmax_number = bc;
                    }
                    //printf("번호 = %d,\tbot값 = %d\t이름 = %s\n", bc, bottomlist[bc], bottomlist_name[bc]);
                    //printf("최고값 번호 = %d\t최고bot값 = %d\t최고값 오브젝트 이름 = %s\n\n", botmax_number, botmax, bottomlist_name[botmax_number]);
                }

//                FILE* ctext = fopen("/home/nano/darknet/detection_text.txt", "w");   젯슨나노 경로
                FILE* ctext = fopen("C:/Users/name3/darknet/darknet-alexeyAB-master/build/darknet/x64/detection_text.txt", "w");
                switch (Distance(botmax)) {
                case 1:
                    fputs("전방 3미터 이내에 ", ctext);
                    break;
                case 2:
                    fputs("전방 2미터 이내에 ", ctext);
                    break;
                default:
                    fputs("3미터 보다 먼 곳에 ", ctext);
                    break;
                }

                fclose(ctext);

                sound(translation(bottomlist_name[botmax_number]));
            }
            else {
                sound("X");
            }
